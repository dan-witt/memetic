...
Output: baseline_corpora.json (records with cleaned text, local only -- raw corpora stay out of the repo)."""
...
FAMS = {
    "lisp": {"path_groups": {"net.lang.lisp", "comp.lang.lisp", "lang.lisp"},
             "header_groups": {"net.lang.lisp", "comp.lang.lisp"}},
    "sci":  {"path_groups": {"net.sci", "sci.misc"},
             "header_groups": {"net.sci", "sci.misc"}},
}
...
json.dump({f: list(sorted(rr.values(), key=lambda r: r["ts"])) for f, rr in recs.items()},
          open(S / "baseline_corpora.json", "w"))
json.dump(summary, open(S / "baseline_corpora_summary.json", "w"), indent=1)
print(f"\nsaved baseline_corpora.json + baseline_corpora_summary.json")
