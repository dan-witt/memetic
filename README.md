...
| How is the square trending, issue over issue? | **Weather report** (recurring): band placement, rolling idea series, register trend, churn signatures, cohort survival, inflows, newcomer refresh, **allocation trend**, **feed lag** | Issue #4 (Aug 14): placement, idea series and register stable; inflow broke its floor (10 new authors/day); allocation still oscillating 0.46–0.55, slope weakening, direction undecidable. **The "door keeps opening" reading is corrected** — a fixed-horizon control finds ~40% of the reported permeability rise is observation-length artefact, leaving a direction of uncertain magnitude. Issue #3's post-publication-**edit** hypothesis is confirmed and instrumented (16 edits landed exactly on the two days whose frozen cells moved); the deferred newcomer-refresh cell is rebuilt with matched pools and a permutation null — no detectable refresh, a hint at best. First non-zero feed backfill (1 item, 2.4 min) revises nothing. | [#1](results/weather/2026-08-11/report.md) · [#2](results/weather/2026-08-12/report.md) · [#3](results/weather/2026-08-13/report.md) · [#4](results/weather/2026-08-14/report.md) |
| Does the square talk about itself more than comp.lang.lisp talked about comp.lang.lisp? | **Allocation**: every claim in all seven pools classified VENUE- vs WORLD-directed (2 classifier families, robustness across normalizers/frames, **human-calibrated** blind gold sample incl. an independent frontier-model rater, κ = 0.76) | **Yes — 2.3–11× every human anchor** (absolute level specification-dependent, 0.31–0.71; human-calibrated contrast ≈ 7× with the published ratios plausibly biased *low*) — and the share **fell nine points across the week**. The relocated self-referentiality construct, measured: idea-diversity normal, register anomalous, allocation anomalous. | [`results/allocation`](results/allocation/report.md) |
...
Current state (sixth pull, 2026-08-15 05:11 UTC): **950 threads; the latest issue,
[`results/weather/2026-08-14`](results/weather/2026-08-14/report.md), uses a hard cutoff of
2026-08-15 00:00 UTC (9,217 items in scope, Aug 5 → Aug 14)**. `data/manifest.json` records the pull provenance (source,
