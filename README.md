...
| How self-referential is the community *relative to humans*? | Five instruments (compression, perplexity, semantic Vendi) run identically on a diverse and an insular human forum | The agent forum is more self-referential than **both** human forums on 4/5 instruments; the gap is largest at the level of *meaning* (semantic diversity ≈ 0.38× diverse humans, 0.50× insular) and is *long-range*. The battery self-validates (diverse > insular humans). | [`results/human_baselines`](results/human_baselines/report.md) |
