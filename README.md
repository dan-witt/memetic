...
| How is the square trending, issue over issue? | **Weather report** (recurring): band placement, rolling idea series, register trend, churn signatures, cohort survival, inflows, newcomer refresh, **allocation trend**, feed lag, label-coverage audit, **and the matched human-platform level** | Issue #7 (Aug 19): **the pre-registered threshold fired.** Venue share **0.4367** — a third consecutive day below both the old 0.456 band floor and the lemmy.world platform interval's 0.4515 lower bound, which is the three-day rule issue #5 set for calling below-human self-allocation *sustained*, and the series' record low. It **survives correcting both sides**: all 83 items the classifier failed to label return the identical string `SUBJECT MATTER` — a WORLD answer in the wrong surface form — so the published series is an **upper bound** (−0.0064 on 08-19) while the frozen comparator's same-parse bias is bounded an order of magnitude smaller (−0.0006), widening the shortfall from 0.936 to 0.924. What it does *not* buy: the comparator's 55.7% meta-tier frame biases *toward* the square reading low, so the rule firing is not independent evidence. Retries proved inert (0 of 73 resolved). Three other readings moved: recruitment hit a record low (**5** new authors, 0.012 newcomer share), firing issue #6's "08-18 was noise" trigger; the twice-dark newcomer cell was replaced by issue #6's pre-registered **pooled window** (423 vs 3,304 items — newcomers sit farther from the incumbent cloud, Δ 0.0092, p = 0.036, while adding no effective diversity, union 1.002); and the sub-forth dip rate **fell to 10.5%** from 42.1% instead of confirming a third issue in the 40s. The shared-prefix assertion **fired for the first time in seven issues** — 3 windows, attributed to one post-publication edit, none crossing the anchor. A previously undisclosed **n ≥ 10 cohort floor** in the permeability control is now stated: it freezes the pre-registered primary horizon indefinitely at current inflow. | [#1](results/weather/2026-08-11/report.md) · [#2](results/weather/2026-08-12/report.md) · [#3](results/weather/2026-08-13/report.md) · [#4](results/weather/2026-08-14/report.md) · [#5](results/weather/2026-08-17/report.md) · [#6](results/weather/2026-08-18/report.md) · [#7](results/weather/2026-08-19/report.md) |
...
Current state (ninth pull, 2026-08-20 02:59 UTC): **1,268 threads; the latest issue,
[`results/weather/2026-08-19`](results/weather/2026-08-19/report.md), uses a hard cutoff of
2026-08-20 00:00 UTC (13,247 items in scope, 529 authors, Aug 5 → Aug 19)**. `data/manifest.json` records the pull provenance (source,
