...
| How is the square trending, issue over issue? | **Weather report** (recurring): band placement, rolling idea series, register trend, churn signatures, cohort survival, inflows, newcomer refresh, **allocation trend**, feed lag, label-coverage audit, **and the matched human-platform level** | Issue #10 (Aug 22): **the square's largest day, and a pre-registered rule satisfied on the day that argues against it.** 08-22 set series records on volume, active authors and arrivals — **2,380 items** (previous high 1,132), **393 active authors**, and **258 new authors**, more than the founding day's 224 — and the arrival clock shows 08-21 and 08-22 are **one event** the calendar boundary splits. Issue #8's pre-registered decider **completed its condition**: the trailing 5-day mean read 0.4465 then **0.4498**, two days below the 0.4515 bound, which is "goes below **and stays**". Its branch conclusion — *"issue #7 was right and issue #8 was the noise"* — is **affirmed as written**. The report then states the other half just as plainly, and labels it an override: the day's own venue share **rose to 0.4653**, **−0.12 counting standard errors** from the lemmy.world platform point (08-21 was −2.33 SE), and the crossing depth fell from 0.62 to **0.23** of the statistic's noise. **58% of the rise is composition** — newcomers 0.4793 against incumbents' 0.4429 (+0.0365, p = 0.0922), the mirror of issue #9, where the same test rejected the compositional story — so the issue adds an **incumbent-only allocation series**: among authors already present the point stayed below the bound while **rising about one counting SE**, which is weaker than "the level held". Both new instruments arrived in the issue that benefits from them, and the report says so. Issue #9's concentration watch item is answered with an instrument: the fixed-span cell collapsed (5-day 89.1 → **61.4**) while new **incumbent-only rows** read 94.3 → **93.2**, so the published move is dilution by arrivals — and issue #8's two-width "concentration is rising" **reading is withdrawn**, though the retrospective artefact claim only holds at 7 days (incumbent-only **fell** 95.0 → 94.5 over #5–#8) while ~40% of the 5-day rise survives the control. The **n ≥ 5 cohort trend retires** by issue #9's own condition (p 0.0352 → 0.1284, moved by one 5-author cohort at 0%); the frozen n ≥ 10 cell **unfreezes at issue #11** when 08-21's 71-author cohort lands. Elsewhere: the first **exact-calendar-day window pair** (bge lisp 1.192 → 1.192; issue #3's decline arm cannot complete before issue #13, its gte arm is live at #11), newcomer Vendi cells now **exclude 1** on 7× the items — precision, not necessarily phenomenon — the dip rate went **4.8% → 26.7%** so that cell is noisier than three issues could show, register moved **+0.0026** on triple traffic (fifth smallest of sixteen daily moves), and backfill hit a series-high **7** on the second-shortest margin, every item minutes old. | [#1](results/weather/2026-08-11/report.md) · [#2](results/weather/2026-08-12/report.md) · [#3](results/weather/2026-08-13/report.md) · [#4](results/weather/2026-08-14/report.md) · [#5](results/weather/2026-08-17/report.md) · [#6](results/weather/2026-08-18/report.md) · [#7](results/weather/2026-08-19/report.md) · [#8](results/weather/2026-08-20/report.md) · [#9](results/weather/2026-08-21/report.md) · [#10](results/weather/2026-08-22/report.md) |
...
Current state (twelfth pull, 2026-08-23 00:52 UTC): **1,682 threads; the latest issue,
[`results/weather/2026-08-22`](results/weather/2026-08-22/report.md), uses a hard cutoff of
2026-08-23 00:00 UTC (17,165 items in scope, 863 authors, Aug 5 → Aug 22)**. `data/manifest.json` records the pull provenance (source,
